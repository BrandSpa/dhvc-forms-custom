...
if(!defined('DHVC_FORM_VERSION'))
	define('DHVC_FORM_VERSION','1.4.15');
...
#dhvcform-'.$post->ID.'.dhvc-form-flat .dhvc-form-textarea textarea{
	...
	private function _form_handler($form_id, $form_controls, $save_entry = true,$send_notice = true, $autoreply = true,$return=false){
		...
		$submited_data = apply_filters('dhvc_form_submited_data', $submited_data);
				...
				if (is_array($file['error'])) {
					...
					if ($file['error'] === UPLOAD_ERR_OK) {
						...
						if(is_string($result) && !empty($result)){
							dhvc_form_add_messages($form_id,$result);
							return false;	
						}
						if (is_array($result)) {
		...
		$entry_data = apply_filters('dhvc_form_entry_data', $entry_data);
		$email_data = apply_filters('dhvc_form_email_data', $email_data);
		...
		if($notice && $send_notice){
			...
			$mailer_from = apply_filters('dhvc_form_notice_sender_email', $mailer_from,$posted_data);
			...
			$recipients = apply_filters('dhvc_form_notice_recipient_email', $recipients);
		...
		if($reply && $autoreply){
			...
			$recipients = apply_filters('dhvc_form_reply_recipient', $recipients);
			if($recipients){
				if(isset($posted_data[$recipients]) && dhvc_form_validate_email($posted_data[$recipients])){
					...
					$reply_FromName = apply_filters('dhvc_form_reply_from_name', $reply_FromName);
					...
					$subject = apply_filters('dhvc_form_reply_from_subject',$subject);
	...
	protected function _mailchimp($data){
		...
		if (!$success ) {
			return array(
				'success' => false,
				'message'  => __( 'Not Subscribe to Mailchimp!','dhvc-form' ),
			);
		} else {
			return array(
				'success' => true,
