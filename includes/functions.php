...
function dhvc_form_phpmailer($to, $subject, $message, $headers = '', $attachments = array()){
	...
	$phpmailer->From = apply_filters( 'dhvc_form_phpmailer_from', $from_email );

	$phpmailer->FromName = apply_filters( 'dhvc_form_phpmailer_from_name', $from_name );
...
function dhvc_form_add_messages($form_id,$message=''){
	return update_post_meta($form_id, '_dhvc_form_messages', $message);
}
...
function dhvc_form_the_messages($form_id){
	...
	if(is_array($messages)){
		$html .= '<ul class="dhvc-form-message-list">';
